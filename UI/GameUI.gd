...
# =========================================================
# Game UI (Optimized & Decoupled)
# =========================================================

# Signals for the Tutorial Manager
signal speed_interacted
signal game_paused
signal game_resumed
signal upgrade_purchased(type: String)
signal player_city_count_changed(new_count: int)
...
func _setup_panels() -> void:
	if side_panel != null: _setup_side_panel_position()
	if side_panel_content != null: side_panel_content.modulate.a = 1.0
	if game_over_panel != null: game_over_panel.visible = false
	if pause_panel != null: pause_panel.visible = false

func _setup_gold_icons() -> void:
	_gold_icon_frames.clear()
	if gold_icon_sheet == null: return
...
func _setup_slider() -> void:
	...
	if send_button_25 != null: send_button_25.pressed.connect(_on_send_25_pressed)
...
func _connect_buttons() -> void:
	...
	if defense_upgrade_button != null: defense_upgrade_button.pressed.connect(_on_upgrade_defense)
	if resume_btn != null: resume_btn.pressed.connect(_on_resume)
	if main_menu_btn_pause != null: main_menu_btn_pause.pressed.connect(_on_main_menu)
	if main_menu_btn_go != null: main_menu_btn_go.pressed.connect(_on_main_menu)
	if speed_slower_btn != null: speed_slower_btn.pressed.connect(_on_speed_slower)
	if speed_faster_btn != null: speed_faster_btn.pressed.connect(_on_speed_faster)
	if pause_btn != null: pause_btn.button_down.connect(_toggle_pause)

func _connect_global_signals() -> void:
	CycleClock.cycle_ticked.connect(_on_cycle_ticked)
	if FactionState != null: FactionState.gold_changed.connect(_on_gold_changed)

# =========================================================
# PUBLIC API
# =========================================================

# Replace the old set_graph_map function in GameUI.gd with this:
func set_graph_map(gm: GraphMap) -> void:
	...
	if graph_map != null and graph_map.level_data != null:
		...
		if "Tutorial" in graph_map.level_data.level_name:
			var tutorial_script = load("res://UI/TutorialManager.gd")
			if tutorial_script:
				# Use .new() for scripts extending Node 
				var tutorial_mgr = tutorial_script.new() 
				add_child(tutorial_mgr)
				tutorial_mgr.setup(self, graph_map)
...
func refresh_all() -> void: _refresh_all_ui()
...
func _on_window_resized() -> void:
	...
	if _side_panel_visible: side_panel.position.x = _get_side_panel_shown_x()
	else: side_panel.position.x = _get_side_panel_hidden_x()
...
func _refresh_selected_city_ui() -> void:
	if _selected_city == null or not is_instance_valid(_selected_city): return
	if _selected_city.data == null: return
...
func _refresh_city_header(d: CityData) -> void:
	if city_name_label != null: city_name_label.text = d.name
	if faction_icon != null:
		match d.owner:
			1: faction_icon.texture = friendly_faction
			2: faction_icon.texture = enemy_faction
			_: faction_icon.texture = neutral_faction
	if owner_label != null:
		match d.owner:
			1:
...
func _refresh_production_cards(d: CityData) -> void:
	_set_production_card(current_troops_card, current_troops_icon_node, current_troops_value, true, current_troops_icon, str(d.army))
	_set_production_card(troop_production_card, troop_production_icon_node, troop_production_value, true, troop_production_icon, "+%d/tick" % d.production_per_cycle)
	_set_production_card(gold_production_card, gold_production_icon_node, gold_production_value, true, gold_production_icon, "+%d/tick" % d.gold_per_cycle)
	_set_production_card(defense_card, defense_icon_node, defense_value, true, defense_icon, "x%.1f" % d.get_defense_multiplier())
...
func _refresh_send_ui(d: CityData, is_player_city: bool) -> void:
	...
	if send_percent != null: send_percent.visible = is_player_city
	if send_amount != null: send_amount.visible = is_player_city
	if is_player_city and send_slider != null: _update_send_preview(send_slider.value, d.army)

func _update_send_preview(value: float, army_count: int) -> void:
	if graph_map != null: graph_map.player_send_ratio = value
...
func _set_production_card(card_root: Control, icon_node: TextureRect, value_label: Label, visible: bool, icon_texture: Texture2D, value_text: String) -> void:
	...
	if icon_node != null: icon_node.texture = icon_texture
	if value_label != null: value_label.text = value_text
...
func _hide_side_panel() -> void:
	...
	_side_panel_tween.tween_callback(func(): side_panel.visible = false)
	_side_panel_visible = false

func _get_side_panel_shown_x() -> float: return get_viewport_rect().size.x - side_panel.size.x
func _get_side_panel_hidden_x() -> float: return get_viewport_rect().size.x
func _kill_side_panel_tween() -> void:
	if _side_panel_tween != null and is_instance_valid(_side_panel_tween): _side_panel_tween.kill()
...
func _on_send_25_pressed() -> void: if send_slider != null: send_slider.value = 0.25
func _on_send_50_pressed() -> void: if send_slider != null: send_slider.value = 0.5
func _on_send_75_pressed() -> void: if send_slider != null: send_slider.value = 0.75
func _on_send_max_pressed() -> void: if send_slider != null: send_slider.value = 1.0
...
func _on_upgrade_production() -> void:
	if _selected_city != null and _selected_city.try_upgrade_production():
		upgrade_purchased.emit("production")
		_refresh_top_bar()
		_refresh_selected_city_ui()

func _on_upgrade_gold() -> void:
	if _selected_city != null and _selected_city.try_upgrade_gold():
		upgrade_purchased.emit("gold")
		_refresh_top_bar()
		_refresh_selected_city_ui()

func _on_upgrade_defense() -> void:
	if _selected_city != null and _selected_city.try_upgrade_defense():
		upgrade_purchased.emit("defense")
		_refresh_top_bar()
		_refresh_selected_city_ui()
...
func _toggle_pause() -> void:
	if game_over_panel != null and game_over_panel.visible: return
	_paused = not _paused
	if _paused: 
	...
	if pause_panel != null: pause_panel.visible = _paused
